        ...
        <div class="tab-content" id="{{ $id }}-tabContent">
            @foreach($insideInputs as $lang => $input)
                <div @class(['tab-pane fade show', 'active' => $lang === \Illuminate\Support\Facades\App::getLocale()]) id="{{ $id }}-{{ $lang }}-content" role="tabpanel" aria-labelledby="{{ $id }}-{{ $lang }}-tab">
                    ...
                    <button type="button" class="btn btn-link" data-click="admin::translate" data-params="{{ json_encode(['lang' => $lang, 'route' => route('admin.translate')]) }}"><i class="fas fa-language"></i> @lang('translate_field_to_lang', ['name' => $name, 'lang' => strtoupper($lang)])</button>
                </div>
            @endforeach
        </div>
    </div>
